...
关于yum为什么能解决依赖关系：
所有的Yum 源里面都有repodata，它里面是有XML格式文件, 用于储存元数据，记录软件包之间的依赖关系。
...
这个在linux基础章节里面说过，有两种命令替换方式，反引号\`命令\` 和$(命令)，前者兼容度更高，但是是比较老的写法了
...
**写时复制 (Btrfs)**：像用**复印机**。你要改原稿上的一个段落，Btrfs 不动原稿，而是：
把你要改的那一小块数据找个**新地方**写一份。
在写完并确认没问题后，把文件的“目录索引”指向这个新地址。
原来的老地址如果没有快照引用，就会被标记为“可回收”。

为什么 Btrfs 默认遵循写时复制？
既然“搬家”这么麻烦，为什么还要默认开启？因为它带来了三个逆天的功能：
1. **数据绝对安全（原子性）**： 由于永远不会覆盖旧数据，Btrfs 要么写入成功，要么维持原样。即使在写入中途断电，你的旧版本文件依然是完整的。这彻底消除了“文件存到一半断电变 0 字节”的悲剧。
2. **瞬间快照（Snapshots）**： 这就是为什么你创建快照只需要 0.1 秒。Btrfs 只需要给当前的账本拍张照。因为旧数据不会被覆盖，快照只需要守着那些“旧地址”不放，文件就永远停留在那个时刻了。
3. **数据压缩与校验**： 因为每次都是写新块，Btrfs 可以在写入前先压缩一下，或者算个校验和（Checksum）。如果是在原位覆盖，计算开销和逻辑复杂度会指数级上升。

btrfs文件系统的COW在每个时期的动作是怎样的，可以参考如下说明
我们用一个比喻：**“透明活页账本”**。硬盘是纸张，文件系统是目录。
1.初始状态
假如有这么一个文件ccc.txt，内容是"hello"
1. 数据层: btrfs在硬盘的1001号格子写下了"hello"
2. 索引层(B-Tree): 账本的索引页写着:ccc.txt -> 指向1001号格子
3. 引用计数: 1001号格子的"关注人数"是1

2.修改动作(COW发生的瞬间)
现在，你想把ccc.txt的内容改成"bye"，在传统文件系统(NTFS/Ext4)里，它会直接在1001号格子把"hello"擦掉，写上"bye"，但btrfs不这样做
1. 寻找新家: Btrfs看到你要修改，它先去硬盘的"荒地"里找了一个空的2002号格子
2. 写入新数: 它在2002号格子里写下"bye"(此时，1001号的格子的"hello"还在，完好无损)
3. 准备切换: 数据写完并校验无误后，Btrfs开始修改索引页
4. 更新索引: 账本索引页更新为: ccc.txt -> 指向2002号格子
5. 旧址处理: 1001号格子的"关注人数"变为0，Btrfs告诉系统，1001号格子现在没用了，以后谁想写东西可以直接覆盖它

3.快照介入
如果你在修改之前，随手点了一下创建快照，这个过程又有了新的变数
1. 复印账本: Btrfs瞬间复印了一份索引页，叫快照1
2. 双重关注: 此时1001号格子的"关注人数"变成了2（当前账本盯着它，快照1也盯着它）
3. 执行修改: 你还是把内容改成了"bye"，写到了2002号格子
4. 索引分家: 主账本指向2002号格子，快照账本依然死死指向1001号格子
