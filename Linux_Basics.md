...
- **1：标准运行法（先授权，再指路）** 说到底，脚本就是一个需要被执行的文件。你必须先赋予它“可执行权限”：`chmod +x xxx.sh`。 然后，你需要明确告诉系统它的位置才能运行（哪怕它就在当前目录）：`./xxx.sh`。
    
- **2：偷懒运行法（调用s解释器）** 如果是写一些用完即弃的临时脚本，其实可以省去修改权限的步骤。 直接把脚本文件当作参数，喂给解释器去读：`bash xxx.sh`。系统会直接运行它，完全不看它有没有可执行权限。
