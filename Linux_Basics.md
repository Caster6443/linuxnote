...
2. **左中括号 `[` 根本不是标点，它是一个“程序”**
    ...
    - **真相：** `[` 是早期 UNIX 系统中 `test` 命令的“马甲”（系统里真实存在一个名为 `[` 的可执行文件）。如果你使用 `which [` 命令，是可以查到它的具体路径的。那么就引入了一个重点，其实if等语句后面跟着的中括号从来就不是什么判断体，而是一个命令，它们看的是这个命令的退出状态码是0(成功)还是非0(失败)，也就是说，我可以写 `if [...]`,我也可以写 `if id nginx`,这个写法中，if实际上就是在看id nginx这个命令的返回值
