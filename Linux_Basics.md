...
- **赋值绝对不能有空格：** 必须写成 `score=85`。一旦有空格（如 `score = 85`），Shell 就会把 `score` 误认作一个要执行的命令，把 `=` 当作它的参数，从而报错“找不到命令”。
    
- **左中括号 `[` 本质不是简单的括号，是一个“程序”：** 真相是，`[` 是早期 UNIX 系统中 `test` 命令的“马甲”（系统里真实存在一个名为 `[` 的可执行文件）， 如果你使用`which [`命令是可以查到这个可执行文件的具体路径的。
    
- **中括号的空格规则：** 既然 `[` 是命令，那么 `-ge`、`5`、`3` 都是传给它的参数。命令与参数之间必须用空格隔开，例如：`[ 5 -ge 3 ]`。
    
- **中括号的闭合规则：** `[` 命令内部有一条硬性规定，传入的最后一个参数必须是 `]`，这仅仅是为了让代码在视觉上显得闭合。
    
- **数字比较符的记忆：** Shell 中的数字比较符（如 `-ge`, `-lt`）看起来毫无规律，但其实它们全部是英文单词首字母的缩写。只要还原成英文，看一眼就能举一反三：
...
- **文件系统探测(给路径查户口)**
  ...
  - **`-e` (Exist)**：只要这个路径**存在**，不管是文件、目录还是快捷方式，都返回真。`[[ -e "/etc/passwd" ]]`
    
- **`-f` (File)**：必须存在，且必须是一个**普通的文本/数据文件**（不能是目录）。`[[ -f "data.txt" ]]`
    
- **`-d` (Directory)**：必须存在，且必须是一个**目录（文件夹）**。`[[ -d "/var/log" ]]`
    
- **`-s` (Size)**：文件存在，并且**大小不为 0**（里面有内容）。常用来检查日志文件是不是空的。`[[ -s "error.log" ]]`
    

#### 2. 字符串状态探测（检查变量里有没有东西）

- **`-z` (Zero)**：判断字符串长度是否为 **0（也就是空的）**。防呆设计必备：`[[ -z "$name" ]]` (如果没输入名字，为真)。
    
- **`-n` (Non-zero)**：判断字符串长度是否**非空（里面有东西）**。`[[ -n "$name" ]]` (如果输入了名字，为真)。
