...
(
    while :; do
        ...
        if ["$status" == "Playing"](%22%24status%22%20%3D%3D%20%22Playing%22.md) || ["$status" == "Paused"](%22%24status%22%20%3D%3D%20%22Paused%22.md); then
...
# 检查录制进程是否在运行
is_running() {
  [-r "$PIDFILE"](-r%20%22%24PIDFILE%22.md) || return 1
  local pid; read -r pid <"$PIDFILE" 2>/dev/null || return 1
  [-n "$pid"](-n%20%22%24pid%22.md) && kill -0 "$pid" 2>/dev/null
}
...
notify() { 
  ...
  if has notify-send; then
    if [-n "$body"](-n%20%22%24body%22.md); then
...
list_render_nodes() {
  ...
  for d in /dev/dri/renderD*; do
    [-r "$d"](-r%20%22%24d%22.md) && printf '%s\n' "$d"
  done 2>/dev/null || true
}
# 显示当前选择的渲染节点
render_display() {
  local cur="${1:-}"
  if [-z "$cur"](-z%20%22%24cur%22.md); then
    msg render_auto
  else
    printf "%s" "$cur"
  fi
...
pick_render_device() {
  ...
  if [-n "$dev" && ! -r "$dev"](-n%20%22%24dev%22%20%26%26%20%21%20-r%20%22%24dev%22.md); then
...
choose_ext(){
  local e="${SAVE_EXT,,}"
  if [| "$e" == "auto"](-z%20%22%24e%22.md); then
    ext_for_codec "$CODEC"
  else
...
# ================== Menus ==================
# 规范化菜单输出
__norm() { printf '%s' "$1" | tr -d '\r' | sed 's/^[:space:](%3Aspace%3A.md)*//;s/[:space:](%3Aspace%3A.md)*$//'; }

# 自动选择菜单后端
_pick_menu_backend() {
  local pref="${MENU_BACKEND,,}"
  case "$pref" in fuzzel|wofi|rofi|bemenu|fzf|term) : ;; auto|"") pref="auto" ;; *) pref="auto" ;; esac
  if ["$pref" != "auto"](%22%24pref%22%20%21%3D%20%22auto%22.md); then
    if has "$pref"; then echo "$pref"; else [-t 0](-t%200.md) && echo "term" || echo "none"; fi
    return
  fi
  for b in fuzzel wofi rofi bemenu fzf; do has "$b" && { echo "$b"; return; }; done
  [-t 0](-t%200.md) && echo "term" || echo "none"
}
...
menu_pick() { # $1:title; items...
  ...
  [| -z "${sel:-}"](%24rc%20-ne%200.md) && return 130
  printf '%s' "$(__norm "$sel")"
}
...
# 决定录制哪个输出
decide_output() {
  if [-n "$OUTPUT"](-n%20%22%24OUTPUT%22.md); then printf '%s' "$OUTPUT"; return 0; fi
  local -a outs; mapfile -t outs < <(list_outputs || true)
  local out_title; out_title="${MENU_TITLE_OUTPUT:-$(msg title_output)}"
  if ["${OUTPUT_SELECT}" == "menu"](%22%24%7BOUTPUT_SELECT%7D%22%20%3D%3D%20%22menu%22.md) || { ["${OUTPUT_SELECT}" == "auto"](%22%24%7BOUTPUT_SELECT%7D%22%20%3D%3D%20%22auto%22.md) && ((${#outs[@]} > 1)); }; then
...
choose_render_menu() {
  ...
  if ["$pick" == "$auto_item"](%22%24pick%22%20%3D%3D%20%22%24auto_item%22.md); then
    DRM_DEVICE=""
    rm -f "$CFG_DRM"
    return 0
  fi
  local sel="$pick"
  if [-n "$sel" && -r "$sel"](-n%20%22%24sel%22%20%26%26%20-r%20%22%24sel%22.md); then
    DRM_DEVICE="$sel"
...
show_settings_menu() {
  while :; do
    ...
    local ff_display; if [| "${SAVE_EXT,,}" == "auto"](-z%20%22%24SAVE_EXT%22.md); then ff_display="$(msg ext_auto)"; else ff_display="$SAVE_EXT"; fi
    ...
    if ["$pick" == "$(msg menu_set_fps "$fps_display")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_set_fps%20%22%24fps_display%22%29%22.md); then
      local newf; newf="$(menu_pick "$(msg title_select_fps)" "60" "30" "120" "144" "165" "240" "$(msg fps_unlimited)")" || continue
      if ["$newf" == "$(msg fps_unlimited)"](%22%24newf%22%20%3D%3D%20%22%24%28msg%20fps_unlimited%29%22.md); then
        FRAMERATE=""; rm -f "$CFG_FPS"
      else
        if [[ "$newf" =~ ^[0-9]+$ && "$newf" -gt 0 ]]; then FRAMERATE="$newf"; printf '%s' "$FRAMERATE" >"$CFG_FPS"; fi
      fi
    elif ["$pick" == "$(msg menu_toggle_sysaudio "$sysaudio_display")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_toggle_sysaudio%20%22%24sysaudio_display%22%29%22.md); then
      if ["$SYS_AUDIO" == "on"](%22%24SYS_AUDIO%22%20%3D%3D%20%22on%22.md); then SYS_AUDIO="off"; else SYS_AUDIO="on"; fi
      printf '%s' "$SYS_AUDIO" >"$CFG_SYS_AUDIO"
    elif ["$pick" == "$(msg menu_toggle_audio "$micaudio_display")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_toggle_audio%20%22%24micaudio_display%22%29%22.md); then
      if ["$MIC_AUDIO" == "on"](%22%24MIC_AUDIO%22%20%3D%3D%20%22on%22.md); then MIC_AUDIO="off"; else MIC_AUDIO="on"; fi
      printf '%s' "$MIC_AUDIO" >"$CFG_MIC_AUDIO"
    elif ["$pick" == "$(msg menu_set_codec "$CODEC")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_set_codec%20%22%24CODEC%22%29%22.md); then
    ...
    elif ["$pick" == "$(msg menu_set_filefmt "$ff_display")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_set_filefmt%20%22%24ff_display%22%29%22.md); then
      choose_filefmt_menu
    elif ["$pick" == "$(msg menu_set_render "$render_display_now")"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_set_render%20%22%24render_display_now%22%29%22.md); then
      choose_render_menu
    elif ["$pick" == "$(msg menu_back)"](%22%24pick%22%20%3D%3D%20%22%24%28msg%20menu_back%29%22.md); then
      return 0
    fi
  done
}
...
decide_mode() {
  ...
  while :; do
    ...
    if   ["$pick" == "$L_FULL"](%22%24pick%22%20%3D%3D%20%22%24L_FULL%22.md); then MODE_DECIDED="full";   return 0
    elif ["$pick" == "$L_REGION"](%22%24pick%22%20%3D%3D%20%22%24L_REGION%22.md); then MODE_DECIDED="region"; return 0
    elif ["$pick" == "$L_SETTINGS"](%22%24pick%22%20%3D%3D%20%22%24L_SETTINGS%22.md); then show_settings_menu; continue
    elif ["$pick" == "$L_EXIT"](%22%24pick%22%20%3D%3D%20%22%24L_EXIT%22.md); then return 130
    else return 130; fi
  done
}
...
start_rec() {
  ...
  if ["$mode" == "full"](%22%24mode%22%20%3D%3D%20%22full%22.md); then TARGET_DIR="$ROOT_DIR/${SAVE_SUBDIR_FS}"; else TARGET_DIR="$ROOT_DIR"; fi
  mkdir -p "$TARGET_DIR"

  # 全屏模式
  if ["$mode" == "full"](%22%24mode%22%20%3D%3D%20%22full%22.md); then
    output="$(decide_output)" || { exit 130; } 
    [-n "$output"](-n%20%22%24output%22.md) && args+=( -o "$output" )
    marker="FS${output:+-$output}"
  # 区域模式
  else
    if [-n "$REC_AREA"](-n%20%22%24REC_AREA%22.md); then
      GEOM="$REC_AREA"
    else
      has slurp || { notify "$(msg err_need_slurp)"; exit 1; }
      set +e; GEOM="$(slurp)"; local rc=$?; set -e
      if [| -z "${GEOM// /}"](%24rc%20-ne%200.md); then notify "$(msg cancel_no_region)"; exit 130; fi
    fi
  ...
  local dev; dev="$(pick_render_device)"; [-n "$dev"](-n%20%22%24dev%22.md) && args+=( -d "$dev" )

  # 音频参数
  local -a audio_args=() 
  if ["$SYS_AUDIO" == "on"](%22%24SYS_AUDIO%22%20%3D%3D%20%22on%22.md); then
      audio_args+=( --audio="@DEFAULT_AUDIO_SINK@.monitor" )
  fi
  if ["$MIC_AUDIO" == "on"](%22%24MIC_AUDIO%22%20%3D%3D%20%22on%22.md); then
  ...
  if ["$CODEC" == *"_vaapi"](%22%24CODEC%22%20%3D%3D%20%2A%22_vaapi%22.md); then
  ...
  # 帧率参数
  if [-n "$FRAMERATE"](-n%20%22%24FRAMERATE%22.md); then
  ...
  # 发送录制开始通知
  local note; if ["$mode" == "full"](%22%24mode%22%20%3D%3D%20%22full%22.md); then note="$(msg notif_started_full "$output" "$SAVE_PATH")"; else note="$(msg notif_started_region "$SAVE_PATH")"; fi
  [-n "$dev"](-n%20%22%24dev%22.md) && note+="$(msg notif_device_suffix "$dev")"
  notify "$note"
}
...
stop_rec() {
  ...
  local save_path=""; [-r "$SAVEPATH_FILE"](-r%20%22%24SAVEPATH_FILE%22.md) && read -r save_path <"$SAVEPATH_FILE"
  if [-n "$save_path" && -f "$save_path"](-n%20%22%24save_path%22%20%26%26%20-f%20%22%24save_path%22.md); then
    ...
    [-n "$err_log"](-n%20%22%24err_log%22.md) && notify "$s" "错误详情: $err_log" || notify "$s"
  fi

  # 
  if ["${PKILL_AFTER_STOP,,}" != "off"](%22%24%7BPKILL_AFTER_STOP%2C%2C%7D%22%20%21%3D%20%22off%22.md); then
...
kernel="$1"
[-n "$kernel"](-n%20%22%24kernel%22.md) || exit 0
 
# use already installed kernel if it exists
[! -f "$KERNELDESTINATION"](%21%20-f%20%22%24KERNELDESTINATION%22.md) || kernel="$KERNELDESTINATION"
...
[Open: Pasted image 20251213135843.png](62676cbb4a42c76b7f395b46c97e51ad_MD5.jpg)
![62676cbb4a42c76b7f395b46c97e51ad_MD5.jpg](62676cbb4a42c76b7f395b46c97e51ad_MD5.jpg)

开机后装上n卡驱动，在设备管理器上可以看到n卡成功安装使用了
[Open: Pasted image 20251213143143.png](099e5e3183ec6f56a47ff67d14f8f207_MD5.jpg)
...
[Open: Pasted image 20251213144357.png](a5b461818005e59b7a9bd18f0bbef7cc_MD5.jpg)
...
配置结束大概是这样
[Open: Pasted image 20251214003246.png](52a72e57902a24011dcd312b0bdf4e83_MD5.jpg)
...
printf "\n${B}确认切换? [y/N]: ${R}"
read CONFIRM
["$CONFIRM" != "y" && "$CONFIRM" != "Y"](%22%24CONFIRM%22%20%21%3D%20%22y%22%20%26%26%20%22%24CONFIRM%22%20%21%3D%20%22Y%22.md) && printf "\n${GRAY}取消。${R}\n" && exit 0
...
if [ $? -eq 0 ]; then
    printf "${R}\n${B}${GREEN}✅ 完成。${R} 请重启。\n"
    read -p "立即重启? [y/N]: " RB
    [| "$RB" == "Y"](%22%24RB%22%20%3D%3D%20%22y%22.md) && reboot
else
    printf "${R}\n${RED}❌ 失败！请检查日志。${R}\n"
    exit 1
fi
...
添加文件系统类型的硬件
[Open: Pasted image 20251217225758.png](3c515fd8863a183782d1c8f03217cd43_MD5.jpg)
...
# archlinux（niri）配置
我的设备信息
[Open: Pasted image 20251205231208.png](05fb4d754cd84c33fdca4e18c3f79d6d_MD5.jpg)
...
[Open: Pasted image 20260115141145.png](3cffcf9af553ff1be660276dffd6b4de_MD5.jpg)
...
start_swayidle() {
    ...
    if [$PID -eq 0](%24PID%20-eq%200.md) || ! kill -0 "$PID" 2>/dev/null; then
        swayidle -w \
...
cleanup() {
    # 如果有 PID，直接杀掉
    if [$PID -ne 0](%24PID%20-ne%200.md); then
...
while true; do
    ...
    if timeout 2s systemd-inhibit --list --no-pager | grep -q "Manually activated by user"; then
        # === 发现抑制锁 ===
        if [$PID -ne 0](%24PID%20-ne%200.md) && kill -0 "$PID" 2>/dev/null; then
            kill "$PID" 2>/dev/null
            PID=0
        fi
    else
...
[Open: Pasted image 20260106114924.png](b79450be15fd37f4bd46d8e4e9e00025_MD5.jpg)
...
## Git仓库推送流程
在github上弄了dotfiles仓库用于个人配置文件存储，项目地址[https://github.com/Caster6443/dotfiles](https%3A//github.com/Caster6443/dotfiles.md)，前置认证流程就不记录了，这里记录一下使用方法
...
如何验证？
`pw-top`命令查看
[Open: Pasted image 20251215220719.png](fac656bba474cf4bdd53348fe1d1c242_MD5.jpg)
